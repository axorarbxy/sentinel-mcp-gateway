...
// Module Pages - Make sure these exist
...
// Navigation wrapper that handles tab switching
const AppLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const navigate = useNavigate();
  const [tabValue, setTabValue] = useState(0);

  // Get current path and set tab
  useEffect(() => {
...
function App() {
  ...
  // If not authenticated, show login/register/landing pages
  if (!isAuthenticated) {
    return (
